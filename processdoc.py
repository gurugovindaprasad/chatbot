...
class AdaptiveTroubleshootingDocumentProcessor:
    ...
    def create_adaptive_splitting_patterns(self, patterns: Dict) -> List[str]:
        ...
        if patterns['custom_markers']:
            # Create pattern from most common custom markers
            common_markers = [item.split(':')[0] for item, count in patterns['custom_markers'][:5]]
            if common_markers:
                marker_pattern = '|'.join(re.escape(marker) for marker in common_markers)
                regex_patterns.append(f r'\n(?=(?:{marker_pattern}):)')
